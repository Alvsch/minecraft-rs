...
fn build_entities() -> anyhow::Result<TokenStream> {
    ...
    for (entity_name, entity) in entities.clone() {
        ...
        if let Some(entity_type) = entity.typ {
            ...
            module_body.extend([quote! {
                #[doc = #bundle_doc]
                #[derive(Debug)] // bevy_ecs::bundle::Bundle
        ...
        for field in &entity.fields {
            ...
            module_body.extend([quote! {
                #[derive(evenio::component::Component, PartialEq, Clone, Debug, ::derive_more::Deref, ::derive_more::DerefMut)]
            ...
            systems.extend([quote! {
                ...
                fn #system_name_ident(
                    mut fetcher: Fetcher<(&#component_path, &mut tracked_data::TrackedData)>,
                ) {
                    for (value, tracked_data) in fetcher.iter_mut() {
        ...
        module_body.extend([quote! {
            #[doc = #marker_doc]
            #[derive(evenio::component::Component, Copy, Clone, Default, Debug)]
        ...
        match entity_name.as_str() {
            ...
            "PlayerEntity" => {
                module_body.extend([quote! {
                    ...
                    #[derive(evenio::component::Component, Copy, Clone, Debug)]
    ...
    systems.extend([quote! {
        ...
        fn update_living_and_player_absorption(
            mut fetcher: Fetcher<(&living::Absorption, &mut player::AbsorptionAmount)>,
        ) {
            for (living_absorption, player_absorption) in fetcher.iter_mut() {
        ...
        fn update_living_attributes(
            ...
            for (tracked, attributes) in fetcher.iter_mut() {
    ...
    Ok(quote! {
        ...
        fn add_tracked_data_systems(world: &mut World) {
            #systems

            #(
                world.add_handler(#system_names);
            )*
        }
    })
}
